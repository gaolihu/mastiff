TOP_DIR=`pwd`
WORK_DIR=$TOP_DIR/build_x64/mastiff/nginx/
CC=`which gcc`
CC_PATH=`dirname $CC`
...
./configure \
    ...
    --with-openssl-opt="no-asm no-async shared zlib-dynamic no-ssl3-method no-ssl2 -DOPENSSL_USE_IPV6  -DOPENSSL_NO_DYNAMIC_ENGINE -I$ZLIB_INSTALL/include -L$ZLIB_INSTALL/lib -lz" \
    --prefix=$TOP_DIR/build_x64/ \
...
sed -i 's|configure --disable-shared  --enable-jit|configure --disable-shared --enable-unicode-properties --enable-pcre16 --enable-pcre32 --enable-jit --enable-pcregrep-libz CC=$(CC) CXX=$(CXX)|g' objs/Makefile
